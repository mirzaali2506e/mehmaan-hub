...
function send_otp_email($toEmail, $toName, $otp) {
    ...
    try {
        ...
        $mail->SMTPSecure = defined('MAIL_ENCRYPTION') ? MAIL_ENCRYPTION : PHPMailer\PHPMailer\PHPMailer::ENCRYPTION_STARTTLS;
        $mail->Port       = defined('MAIL_PORT') ? (int)MAIL_PORT : 587;

        $mail->setFrom(defined('MAIL_FROM_ADDRESS') ? MAIL_FROM_ADDRESS : 'no-reply@mehmaanhub.com', SITE_NAME);
